...
//身份证号输入限制
- (BOOL)interVerifyIdentityCardNum
{
    NSString *regex2 = @"[xX0-9]*";
    return [self isValidateByRegex:regex2];
}
/**
 *  字母、文字、数字
 */
- (BOOL)isChineseNumberOrLetter{
    NSString *regex = @"[a-zA-Z0-9\u4e00-\u9fa5| ]+";
    BOOL status = [self isValidateByRegex:regex];
    NSString *other = @"➋➌➍➎➏➐➑➒";
    BOOL nineKeyBoard = [other rangeOfString:self].location != NSNotFound;
    if (status || nineKeyBoard) {
        return YES;
    }
    else
        return NO;
...
- (BOOL)isRealNamePredicate{
 
    NSString *realNamePattern = @"^[\u4e00-\u9fa5|·]{0,}";//名字
     return [self isValidateByRegex:realNamePattern];
}
